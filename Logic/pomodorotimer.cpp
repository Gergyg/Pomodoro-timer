...
PomodoroTimer::PomodoroTimer(int workTime, int shortBreakTime, int longBreakTime, int rounds, int roundsForLongBreak, string activity, int id, string user) :
    workTime(workTime),
...
void PomodoroTimer::startTimer(){
    ...
    while(completedRounds < rounds){
        ...
        if(ending) break;
        completedRounds++;
        Statistics::getInstance()->updateCompletedPomodoros(user, activity);
        Statistics::getInstance()->updateWorkedTime(user, activity, workTime);
        ...
        timer(rTime);
        if(ending) break;
        Statistics::getInstance()->updateRestTime(user, activity, rTime);
    }

}

tuple<string, string, int, int, int, int, int, int> PomodoroTimer::getTuple(){
    return make_tuple(activity, user, id, workTime, shortBreakTime, longBreakTime, rounds, roundsForLongBreak);
}
